...
<head>
		...
		<title><?php if(!isset($_SESSION['owner_id'])){echo 'Error';} else{echo 'Profile';}?></title>
...
<body>
	...
	<?php
		...
		if(!isset($_SESSION['owner_id'])){
			...
			$result = $controller->invoke($_SESSION['owner_id']); //get owner details
			...
			if(is_null($result)){
				...
				if(isset($_GET['update'])){
					...
					if($update == 0){
						...
						echo "<div class='text-center text-success'><strong>Update Successful !</strong></div>";
					}
				}
?>	
	...
	<table class="table table-hover">
		<tr>
			...
			echo '
			<img id="profile_picture" src="'.HTTP.'user/owner/resources/images/'.$name.'/'.$name.'.jpg" alt="test profile image"/>
			';
			#profile picture in the images folder of owner inside folder name same as owner name(with no spaces in between)
		...
		<?php
			...
			echo '<tr><td>Contact No<td><td>'.$result[$index++].'</td></tr>';
		?>
	</table>
	</div>
<?php
